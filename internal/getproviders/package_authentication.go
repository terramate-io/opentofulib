...
func (s signatureAuthentication) AuthenticatePackage(location PackageLocation) (*PackageAuthenticationResult, error) {
	...
	if !shouldValidate {
		...
		log.Printf("[WARN] Skipping GPG validation of provider package %s as no keys were provided by the registry. See https://github.com/terramate-io/opentofulib/pull/309 for more information.", location)
